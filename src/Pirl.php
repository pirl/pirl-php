...
/**
 * Pirl JsonRPC API for PHP.
 *
 * Implements Pirl JsonRPC API for PHP
 *   https://github.com/pirl/wiki/wiki/JSON-RPC.
 *
 * This part of the Drupal Pirl Module:
 * https://groups.drupal.org/pirl
 *
 * Pirl class is based on ethjs-schema by Nick Dodson.
 *   https://github.com/digitaldonkey/ethjs-schema
 */
class Pirl extends PirlStatic {
  ...
  /**
   * Constructing Pirl Class.
   *
   * Remarks:
   * Pirl class is based on ethjs-schema.json file.
   * - Everything is typed. All data types extend PirlDataType class.
   * - Primitive data have a mostly common interface. See PirlDataTypePrimitive.
      ...
      ${$name} = function () {
        ...
        if (count($args) && isset($argument_class_names)) {
          ...
          foreach ($args as $i => $arg) {
            ...
            if ($argument_class_names[$i] !== $arg->getType()) {
              ...
              // are not data like in pirl_getBlockByHash().
              if ($arg->isPrimitive() && $arg->getType() !== 'PirlB') {
                $request_params[] = $arg->hexVal();
              }
              elseif ($arg->isPrimitive() && $arg->getType() === 'PirlB') {
        ...
        if ($require_default_block) {
          ...
          foreach ($argument_class_names as $i => $class) {
            if ($class === 'PirlBlockParam' && !isset($request_params[$i])) {
        ...
        $is_primitive = (bool) PirlDataTypePrimitive::typeMap($return_type);

        if (is_array($return_type)) {
          $return_type_class = array(PirlDataTypePrimitive::typeMap($return_type[0]));
        }
        elseif ($is_primitive) {
          $return_type_class = PirlDataTypePrimitive::typeMap($return_type);
        }
        else {
          // Return Complex type.
        ...
        if (file_exists($workaround_file)) {
          ...
          $value = call_user_func('pirl_workaround_' . $method, $value);
        }
  ...
  public function createReturnValue($value, $return_type_class, $method) {
    ...
    $class_name = '\\Pirl\\' . PirlDataType::getTypeClass($return_type_class);
    // Is array ?
    ...
    elseif (!$is_primitive) {
      ...
      // Like a Null Object? Should PirlDataTypes have a test if they are valid?
   ...
   *
   * See:
   * https://github.com/pirl/wiki/wiki/Pirl-Contract-ABI#function-selector
   *
   * @param string $input
   *   Method signature.
   *
  ...
  public function getMethodSignature($input) {
    if ($this->isValidFunction($input)) {
      $keccac = $this->web3_sha3(new PirlS($input));
