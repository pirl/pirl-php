...
foreach ($schema['objects'] as $obj_name => $params) {
  ...
  $data = array(
    ...
    " */",
    "class $obj_name extends PirlDataType {",
    "",
    $properties,
    "  /**",
    "   * Constructor.",
...
function makeSetFunctions(array $input) {
  ...
  foreach ($input['params'] as $name => $type) {
    $functions .= '  public function set' . ucfirst($name) . '(' . PirlDataType::getTypeClass($type, TRUE) . ' $value){' . "\n";
    $functions .= '    if (is_object($value) && is_a($value, \'' . PirlDataType::getTypeClass($type, TRUE) . "')) {\n";
    $functions .= '      $this->' . $name . ' = $value;' . "\n";
    $functions .= '    }' . "\n";
    $functions .= '    else {' . "\n";
    $functions .= '      $this->' . $name . ' = new ' . PirlDataType::getTypeClass($type, TRUE) . '($value);' . "\n";
    $functions .= '    }' . "\n";
    $functions .= "  }\n\n";
  }
  return $functions;
...
function makeReturnArray(Array $input) {
  ...
  foreach ($input['params'] as $name => $type) {
    ...
    if (is_array($type)) {
      $array .= '      (!is_null($this->' . $name . ')) ? $return[' . "'$name'" . '] = PirlStatic::valueArray($this->' . $name . ", '" . $type[0] . "') : array(); \n";
    }
    else {
...
function makeTypeArray(Array $input) {
  ...
  foreach ($input['params'] as $name => $type) {
    if (is_array($type)) {
      $data[] = "      '" . $name . "' => '" . PirlDataTypePrimitive::typeMap($type[0]) . "',";
    }
    else {
      $data[] = "      '" . $name . "' => '" . PirlDataTypePrimitive::typeMap($type) . "',";
    }
  }
  $data[] = '    );';
  $data[] = '  }';
...
function makeConstructor(Array $input) {
  ...
  foreach ($input['params'] as $name => $type) {
    ...
    if (!is_array($type)) {
      $constructor .= PirlDataTypePrimitive::typeMap($type) . ' $' . $name;
